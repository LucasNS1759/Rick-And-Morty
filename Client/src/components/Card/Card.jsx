  ...
  const handlerPostFav = async () => {
    try {
      ...
      let favorites = await axios.get(
        `http://localhost:3001/favorites?nickName=${JSON.parse(nickName)}`
      );

      let coincidencias = favorites.data.filter((fav) => fav.image === props.image);
  ...
  return (
    ...
    <div className="group max-w-[400px] max-h-[400px] relative">
      ...
      <div className="bg-gray-800 absolute inset-0 flex flex-col items-center justify-center text-white opacity-0 transition-all duration-500 transform-gpu group-hover:opacity-100">
        <button
          onClick={() => handlerPostFav()}
          className="absolute top-4 right-4 p-2 rounded-full bg-black hover:bg-gray-200"
        >
          <svg
          ...
          >
            <path
              fillRule="evenodd"
              d="M10 17.178L3.187 20l1.621-7.493L.36 7.822l7.48-.645L10 1.545l2.16 5.632 7.48.645-4.448 3.685 1.622 7.493z"
              clipRule="evenodd"
            />
          </svg>
        </button>
